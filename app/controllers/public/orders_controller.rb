class Public::OrdersController < ApplicationController
  ...
  def check
    ...
    if params[:order][:address_type] == 'self'
      ...
      address = DeliveryDestination.new(postal_code: params[:order][:postal_code], address: params[:order][:address], reciever_name: params[:order][:reciever_name])
  ...
  def create
    ...
    if @order.save
        @cart_items = current_member.cart_items
        @delivery_destination = current_member.delivery_destinations.new
        @delivery_destination.postal_code = @order.postal_code
        @delivery_destination.address = @order.address
        @delivery_destination.reciever_name = @order.reciever_name
        if current_member.registered?(@delivery_destination)
        else
  ...
  def order_params
    params.require(:order).permit(:payment_method, :postal_code, :address, :reciever_name, :biling_amount)
  end
end
